...
export default function Main() {
  ...
  const [data, setData] = useState([
    {
      name: "series-1",
      data: [0.0, 0.01, 0.01, 0.01, 10.01, 0.01, 0.01, 0.01, 0.01, 100.01],
    },
  ]);
  useEffect(() => {
    Init();
    ...
    // Loader(
    //   "https://ipfs.io/ipfs/QmRayg561oGQiL8jMHmy5g1ZWeACQ7gHHf4DdNZ84tU3Lp"
    // ).then((gltf) => {
    //   console.log(gltf);
    //   scene.add(gltf);
  ...
  function ElevationControl(value) {
    return () =>
      elevationController.setValue(elevationController.getValue() + value);
  }

  function AzimuthControl(value) {
    return () =>
      azimuthController.setValue(azimuthController.getValue() + value);
  }
  ...
  function Init() {
    ...
    elevationController = folderSky
      .add(parameters, "elevation", 0, 90, 0.1)
      .onChange(updateSun);
    azimuthController = folderSky
